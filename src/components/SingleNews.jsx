...
const SingleNews = ({ newsList }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 md:gap-10 flex-wrap ">
      {newsList?.map((news, index) => {
        const { url, name, description, image, provider, datePublished } = news;
        return (
          <div key={index + datePublished} className="max-w-sm md:max-w-xl h-auto md:h-80 rounded overflow-hidden shadow-lg bg-primary-3 text-primary-2 bg-opacity-20">
